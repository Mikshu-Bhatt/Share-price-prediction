...
def preprocess_function_original_sp(tokenizer, max_length=256):
    def preprocess_function(examples):
        desc = [f"{d}" for d in examples['desc']]
        desc_ids = tokenizer(desc,
                             add_special_tokens=True,
...
def preprocess_function_original_sp_custom(tokenizer, max_length=256):
    def preprocess_function(examples):
...
def preprocess_function_generator_sp_custom(tokenizer, ignore_index=-100, max_length=256):
    def preprocess_function(examples):
...
def preprocess_test_function_generator_sp(tokenizer, max_length=256):
    def preprocess_function(examples):
        prompts = [f"Starting from wormhole #1, how much dark matter do you need at " \
                   f"minimum to reach wormhole #2?" for dest in examples['dest']]
#         print(examples['desc'])
...
def preprocess_test_function_generator_sp_custom(tokenizer, max_length=256):
    def preprocess_function(examples):
        prompts = [f"which node user is on right now: " + dec for dec in examples['node_feat']]
#         print(examples['desc'])
        # completion = [f"{l}" for l in examples['label']]
